...
class CSP: 
	...
	def check_consistency(self, X, x):
		...
		for Y in self.problem.neighbors[X]:
			if len(self.problem.domains[Y]) == 1 and not self.problem.constraints(X, x, Y, self.problem.domains[Y][0]):
	...
	def get_unassigned_variables(self, X = None):
		...
		if X is None:
			...
			return [x for x in self.problem.neighbors[X] if len(self.problem.domains[x]) > 1]
	...
	def order_by_least_constraining_value(self, X):
		...
		def compare(x1, x2):	
			...
			return sum([not self.problem.constraints(X, x1, Y, y) for Y in self.problem.neighbors[X] for y in self.problem.domains[Y]]) \
				<  sum([not self.problem.constraints(X, x2, Y, y) for Y in self.problem.neighbors[X] for y in self.problem.domains[Y]])
	...
	def clear_inconsistent_values(self, X, Y):
		...
		for x in self.problem.domains[X]:
			if not any([self.problem.constraints(X, x, Y, y) for y in self.problem.domains[Y]]):
